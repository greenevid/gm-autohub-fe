...
export default function PenerimaanBarangBaruPage() {
  ...
  function handleAddItem() {
    if (!pickedBarang || !pickLokasi || (Number(pickJumlah) || 0) <= 0) return;
    setItems((prev) => [
      ...prev,
      {
  ...
  async function handleSubmit(status: "draft" | "terposting") {
    ...
    setError(null);
    try {
      await api.createPenerimaanBarang({
        alasan,
      ...
      setError(err instanceof Error ? err.message : "Gagal menyimpan penerimaan barang");
    } finally {
      setSubmitting(false);
    }
  }
  ...
  return (
    <div className="flex-1 space-y-6 px-4 py-5 sm:px-8 sm:py-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-zinc-900">Penerimaan Barang Baru</h1>
          <p className="text-sm text-zinc-500">Buat penerimaan barang baru</p>
        </div>
        <button
          type="button"
          onClick={() => router.push("/manajemen-stok/penerimaan-barang")}
      ...
      <div className="space-y-4 rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        <label className="block">
          ...
          <input
            ...
            placeholder="Contoh: Barang ditemukan saat stock opname"
      ...
      <div className="rounded-xl border border-zinc-200 bg-white p-5 shadow-sm">
        ...
        <div className="grid grid-cols-1 gap-3 rounded-lg bg-zinc-50 p-4 sm:grid-cols-4">
          ...
          <label className="block">
            <span className="mb-1.5 block text-xs font-medium text-zinc-500">Jumlah</span>
            <input
              type="number"
              min={0}
              value={pickJumlah}
          ...
          <div className="flex items-end sm:col-span-1">
            <button
              ...
              disabled={!pickedBarang || !pickLokasi || (Number(pickJumlah) || 0) <= 0}
